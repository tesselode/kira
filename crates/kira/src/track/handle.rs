...
pub struct TrackHandle {
	...
	pub(crate) volume_change_command_writer: CommandWriter<ValueChangeCommand<Volume>>,
	pub(crate) route_volume_change_command_writers:
...
impl TrackHandle {
	...
	pub fn set_volume(&mut self, volume: impl Into<Value<Volume>>, tween: Tween) {
		self.volume_change_command_writer.write(ValueChangeCommand {
			target: volume.into(),
			tween,
		})
	}
	...
	pub fn set_route(
		...
		let to = to.into();
		self.route_volume_change_command_writers
			.get_mut(&to)
