...
impl<T: Tweenable> Parameter<T> {
	...
	pub fn read_commands(&mut self, command_reader: &mut CommandReader<ValueChangeCommand<T>>)
	where
		T: Send,
	{
		if let Some(ValueChangeCommand { target, tween }) = command_reader.read().copied() {
