use kira::{
	backend::DefaultBackend, effect::doppler::DopplerBuilder, sound::static_sound::StaticSoundData,
	track::SpatialTrackBuilder, AudioManager, AudioManagerSettings, Tween,
};
use macroquad::prelude::*;

const MOVE_SPEED: f32 = 48.0;
...
async fn main() {
	...
	let mut spatial_track = audio_manager
		.add_spatial_sub_track(
			...
			SpatialTrackBuilder::new().with_effect(DopplerBuilder::new()),
		)
		.unwrap();
	spatial_track
		.play(
			StaticSoundData::from_file("crates/examples/assets/vespa-driving-68859.mp3")
				.unwrap()
				.loop_region(..),
		)
		.unwrap();
