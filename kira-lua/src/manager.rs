...
use crate::{
	error::KiraLuaError, event::CustomEvent, event::LEvent, instance::LInstanceId,
	...
	fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
		...
		methods.add_method_mut(
			...
			|_: &Lua, this: &mut Self, (sound_id, settings): (LSoundId, LInstanceSettings)| {
				match this.0.play(sound_id.0, settings.0) {
		...
		methods.add_method_mut(
			"setInstanceVolume",
			|_: &Lua, this: &mut Self, (id, volume): (LInstanceId, LValue<f64>)| match this
				.0
		...
		methods.add_method_mut(
			"setInstancePitch",
			|_: &Lua, this: &mut Self, (id, pitch): (LInstanceId, LValue<f64>)| match this
				.0
